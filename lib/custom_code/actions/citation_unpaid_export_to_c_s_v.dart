...
Future<void> citationUnpaidExportToCSV(BuildContext context) async {
  try {
    // Get data from Firebase Firestore
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        .collection('citation')
        .where('receipt_status', isEqualTo: false)
        .get();

    if (querySnapshot.docs.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('No unpaid citation data found'),
    ...
    html.AnchorElement(href: url)
      ..setAttribute('download',
          'Unpaidcitation_export_${DateTime.now().millisecondsSinceEpoch}.csv')
      ..click();
